...
def test_authenticated_user_can_create_order(
    ...
    order_data = OrderInputSchemaFactory.build(user_id=db_user.id, product_ids=[db_products[1].id])
...
def test_authenticated_user_can_update_order(
    ...
    update_data = OrderInputSchemaFactory.build(user_id=db_user.id, product_ids=[db_products[2].id])
    response = sync_client.patch(f"orders/{db_orders[2].id}", data=update_data.json(), headers=auth_headers)
