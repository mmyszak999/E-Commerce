...
def test_raise_exception_when_cart_item_updated_with_nonexistent_cart_id(
    ...
    cart_item_input = CartItemUpdateSchemaFactory().generate()
    with pytest.raises(DoesNotExist):
        update_cart_item(sync_session, cart_item_input, cart_item_id=db_cart_items[0].id, cart_id=generate_uuid())
...
def test_raise_exception_when_there_is_no_cart_item_with_provided_id(
    ...
    with pytest.raises(NoSuchItemInCartException):
        update_cart_item(sync_session, cart_item_input, cart_item_id=db_cart_items[2].id, cart_id=db_carts.results[0].id)
...
def test_cart_will_contain_correct_total_items_price_after_updating_cart_item(
    ...
    cart = db_carts.results[0]
    item_quantity, cart_item = 100, db_cart_items[1]
...
def test_cart_will_be_deleted_when_there_are_no_cart_items_left(
    sync_session: Session, db_cart_items: list[CartItemOutputSchema], 
    db_carts: list[CartOutputSchema], db_products: list[ProductOutputSchema]
):
    with pytest.raises(EmptyCartException):
        cart = db_carts.results[0]
        cart_item = db_cart_items[1]
