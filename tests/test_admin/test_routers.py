...
def test_superuser_can_grant_staff_permissions(
    ...
    assert response.status_code == status.HTTP_200_OK
    assert (
        response.json()["message"] == "Staff status has been granted successfully"
    )
...
def test_superuser_can_revoke_staff_permissions(
    ...
    assert response.status_code == status.HTTP_200_OK
    assert (
        response.json()["message"] == "Staff status has been revoked successfully"
    )
    assert db_staff_user.is_staff == False
