...
def test_users_can_be_sorted_by_their_attributes(
    ...
    new_user_2 = UserRegisterSchemaFactory().generate(email="zoomer2@mail.com", last_name="zimmermann")
    ...
    response = sync_client.get("users/?sort=last_name__desc,email__desc", headers=staff_auth_headers)
...
def test_categories_can_be_sorted_by_their_attributes(
    ...
    response = sync_client.get("categories/?sort=name__desc", headers=staff_auth_headers)
    assert response.status_code == status.HTTP_200_OK
...
def test_products_can_be_sorted_by_their_attributes(
    ...
    response = sync_client.post("categories/", data=new_category.json(), headers=staff_auth_headers)
    assert response.status_code == status.HTTP_201_CREATED
    
    response = sync_client.get(f"categories/?name__eq={new_category.name}", headers=staff_auth_headers)
    assert response.status_code == status.HTTP_200_OK
