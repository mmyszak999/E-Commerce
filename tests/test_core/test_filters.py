...
from tests.test_core.conftest import db_user, db_staff_user, db_categories, db_products
...
def test_users_can_be_filtered_by_the_attributes(
    ...
    response = sync_client.get(f"users/?username__eq={new_user.username}", headers=staff_auth_headers)
    assert response.status_code == status.HTTP_200_OK
    assert response.json()['total'] == 1
    
    response = sync_client.get(f"users/?email__ge={new_user.email}&email__le=superuser@mail.com", headers=staff_auth_headers)
    assert response.status_code == status.HTTP_200_OK
