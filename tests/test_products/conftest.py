...
DB_PRODUCT_SCHEMAS = [ProductInputSchemaFactory.build() for category in range(3)]
...
def db_products(sync_session: Session, db_categories):
    for product in DB_PRODUCT_SCHEMAS:
        category_id = db_categories[DB_PRODUCT_SCHEMAS.index(product)].id # gets a category with the same id as the schema list index
        product.category_ids = [category_id] # assign the category to the product input schema, one category per product as default
