...
def test_authenticated_user_can_create_product(
    ...
    response = sync_client.post("products/", data=product_data.json(), headers=auth_headers)
...
def test_authenticated_user_can_update_product(
    ...
    update_data = ProductInputSchemaFactory.build(name="test_name", price=14.88, category_ids=[db_categories[0].id])
    response = sync_client.patch(f"products/{db_products[0].id}", data=update_data.json(), headers=auth_headers)
