...
def test_authenticated_user_can_create_product(
    ...
    response = sync_client.post("products/", json=post_product, headers=access_token)
...
def test_authenticated_user_can_update_product(
    ...
    response = sync_client.patch(f"products/{db_products[0].id}", json=update_product, headers=access_token)
