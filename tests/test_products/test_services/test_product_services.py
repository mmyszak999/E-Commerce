...
def test_product_quantity_cannot_be_lower_than_the_items_amount_in_the_carts(
    sync_session, db_products: list[ProductOutputSchema], db_user: UserOutputSchema,
    db_staff_user: UserOutputSchema, db_categories: list[CategoryOutputSchema]
):
    ...
    cart_item_input_1 = CartItemInputSchemaFactory().generate(quantity=cart_item_product_quantity, product_id=product.id)
