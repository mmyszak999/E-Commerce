...
def test_passwords_are_not_identical(incorrect_password):
    with pytest.raises(ValidationError):
        user_data = UserRegisterSchemaFactory.build(password="password", password_repeat=incorrect_password)
...
def test_date_is_from_future(future_date):
    with pytest.raises(ValidationError):
        user_data = UserRegisterSchemaFactory.build(birth_date=future_date, password="password", password_repeat="password")
        UserRegisterSchema(**user_data)
