...
from src.core.utils import check_field_values, confirm_token, if_exists, send_email, generate_confirm_token
...
def change_email_service(
    email_update_schema: EmailUpdateSchema, request_user_email: str,
    background_tasks: BackgroundTasks, session: Session
    ) -> None:
    check_field_values(
        request_user_email,email_update_schema.email, "Please type your current mail address in the 'email' field!"
    )
